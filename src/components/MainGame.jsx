  ...
  return (
    <div className="flex-column animate-slide-up" style={{ height: '100%', padding: '10px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <div className="glass-panel text-center mb-1" style={{ padding: '0.8rem', flexShrink: 0, position: 'relative' }}>
        ...
        {(economyCycle?.phase !== 'normal' || networking > 0) && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '4px', flexWrap: 'wrap' }}>
            {economyCycle?.phase === 'boom' && (
              <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '10px', background: 'rgba(16, 185, 129, 0.25)', color: '#34d399', fontWeight: 'bold' }}>📈 Boom</span>
            )}
            {economyCycle?.phase === 'recession' && (
              <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '10px', background: 'rgba(239, 68, 68, 0.25)', color: '#fca5a5', fontWeight: 'bold' }}>📉 Recession</span>
            )}
            {networking > 0 && (
              <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '10px', background: 'rgba(139, 92, 246, 0.2)', color: '#a78bfa' }}>🤝 Network: {networking}/100</span>
            )}
          </div>
        )}
      </div>
